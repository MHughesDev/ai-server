...
- `docs/OPERATIONS/Production-Deployment-Guide.md` — trim claims that exceed verified behavior.
- Confirm root `README.md` lists this file (and `AGENTS.md` points here); add to `docs/ARCHITECTURE/Overview.md` only if you want extra discovery.
...
| Workflows / engines | **Partial** | Policy **`allowed_pipelines`** is driven by **`listRegisteredWorkflowIds()`** (aligned with registry); still validate router selection + integration tests per workflow. |
...
| Config / feature flags | **Ready** | `src/config/schema.ts`, `docs/SPEC/20_Config_and_FeatureFlags.md` (flag enforcement matrix, 2026-03-24); rollout default safe; note static vs dynamic flag reads. |
...
| Deployment guide | `docs/OPERATIONS/Production-Deployment-Guide.md` |
...
- [ ] **Discoverability** — link to this doc from `README.md` or `docs/ARCHITECTURE/Overview.md` so new readers find it.
...
- [ ] **Async / jobs** — duplicate-submit behavior: optional **`Idempotency-Key`** on **`POST /v1/query/async`** (see `docs/SPEC/02_API_Contracts.md` § Execution model); in-process only unless you add a shared store for multi-replica; job timeouts / stuck-job handling documented for your queue config.
