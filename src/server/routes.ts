...
function getRequestProcessingTimeoutMs(): number {
...
async function withProcessingOrTimeout<T>(
  req: IncomingMessage,
  res: ServerResponse,
  work: Promise<T>
): Promise<T | null> {
...
async function processRequest(
  ...
  if (method === "GET" && path === "/metrics") {
    ...
    const metricsDone = await withProcessingOrTimeout(
      req,
      res,
      (async () => {
        const { getCounterSnapshot, getHistogramSnapshot, getPrometheusText } = await import(
  ...
  if (method === "GET" && path === "/v1/version") {
    ...
    const versionPayload = await withProcessingOrTimeout(
      req,
      res,
  ...
  if (path.startsWith("/admin/flags")) {
    ...
    if (method === "POST" && path === "/admin/flags/evaluate") {
      ...
      const result = await withProcessingOrTimeout(
        req,
        res,
        Promise.resolve(
          flagService.evaluateFlag(body.flag_name as keyof FeatureFlags, {
    ...
    if (method === "GET" && path.startsWith("/admin/flags/overrides/")) {
      ...
      if (parts.length === 2) {
        ...
        const overrides = await withProcessingOrTimeout(
          req,
          res,
          Promise.resolve(flagService.getOverrides(scope as FlagScope, scopeId))
        );
...
function sendRequestProcessingTimeout(
  req: IncomingMessage,
  res: ServerResponse,
  err: RequestProcessingTimeoutError
): void {
  sendJson(res, 504, {
    request_id: (req.headers[HEADER_REQUEST_ID] as string) || null,
    status: "error",
