...
function mergeConfig(target: Record<string, unknown>, source: Record<string, unknown>): void {
  for (const key of Object.keys(source)) {
    const v = source[key];
    if (v != null && typeof v === "object" && !Array.isArray(v) && key in target && typeof (target as Record<string, unknown>)[key] === "object") {
      mergeConfig((target as Record<string, unknown>)[key] as Record<string, unknown>, v as Record<string, unknown>);
    } else if (v !== undefined) {
  ...
  const raw = {
    ...
    flags: {
      ...
      control_plane_enforcement_enabled:
        process.env.CONTROL_PLANE_ENFORCEMENT_ENABLED !== "false" &&
        process.env.NODE_ENV !== "production",
