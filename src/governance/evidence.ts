...
export function validateEvidence(
  ...
  options: { maxStaleDays?: number; requireAllDimensions?: boolean } = {}
): ValidationResult {
  const { maxStaleDays = DEFAULT_STALE_DAYS, requireAllDimensions = true } = options;
  const errors: string[] = [];
  const covered = new Set<ReadinessDimension>(
    evidence.filter((e) => e.valid).map((e) => e.dimension)
  );
  const missingDimensions = requireAllDimensions
    ? READINESS_DIMENSIONS.filter((d) => !covered.has(d))
    : [];
  const completenessPercent =
    READINESS_DIMENSIONS.length > 0
  ...
  for (const e of evidence) {
    ...
    if (e.collectedAtIso) {
      ...
      if (Number.isNaN(t)) errors.push(`Evidence ${e.id}: invalid collectedAtIso`);
      else if (t < cutoff) staleCount++;
    }
  }
