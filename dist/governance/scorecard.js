 ...
 */
export function computeCategoryScore(dimension, evidence, weight) {
 ...
 */
export function computeScorecard(evidence, risks, thresholds = DEFAULT_THRESHOLDS) {
    const weights = { ...DEFAULT_WEIGHTS, ...thresholds.dimensionWeights };
    const categoryScores = READINESS_DIMENSIONS.map((dim) => computeCategoryScore(dim, evidence, weights[dim] ?? 0.15));
    let weightedTotal = 0;
    let totalWeight = 0;
    ...
    const dimensionsWithEvidence = new Set(evidence.filter((e) => e.valid).map((e) => e.dimension))
        .size;
    const evidenceCompletenessPercent = requiredEvidenceCount > 0
        ? Math.min(100, Math.round((dimensionsWithEvidence / requiredEvidenceCount) * 100))
        : 0;
    const criticalGapsCount = risks.filter((r) => r.disposition === "open" && r.dimension === "security").length;
    const openGaps = risks.filter((r) => r.disposition === "open").length;
    const passed = weightedTotal >= thresholds.minWeightedTotal &&
 ...
 */
export function meetsWeightedThreshold(weightedTotal, thresholds = DEFAULT_THRESHOLDS) {
    return weightedTotal >= thresholds.minWeightedTotal;
}
/**
 * Check if evidence completeness meets the minimum threshold.
 */
export function meetsEvidenceCompletenessThreshold(evidenceCompletenessPercent, thresholds = DEFAULT_THRESHOLDS) {
