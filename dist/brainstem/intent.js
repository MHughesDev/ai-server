 ...
 */
function attachmentCapabilityRequired(canonical) {
 ...
 */
export function extractIntent(canonical) {
    const text = (canonical.text ?? "").toLowerCase();
    const toolHint = /use (a )?tool|run (a )?tool|call (a )?tool|invoke (a )?tool|tool (to|for)/i.test(text);
    const researchHint = /\bresearch\b|deep research|find out about|investigate|what (is|are) .+ (and|or) how/i.test(text);
    const decisionHint = /\bdecide\b|choose between|which (option|one)|recommend|should i (pick|choose)|compare (the )?options/i.test(text);
    const routingHints = ["reactive_chat"];
    if (toolHint)
        routingHints.push("coding_agent");
    if (researchHint)
        routingHints.push("deep_research");
    if (decisionHint)
        routingHints.push("decision");
    if (/\bcomposite\b|nested workflow|workflow_call/i.test(text))
        routingHints.push("composite_example");
