...
export class AllowlistToolGateway {
    options;
    constructor(options) {
    ...
    async invoke(request) {
        ...
        if (timeoutMs != null && timeoutMs > 0) {
            ...
            try {
                ...
                const message = err instanceof Error && err.message === "TOOL_TIMEOUT"
...
export class StubAllowedToolGateway {
    resultPayload;
    constructor(resultPayload = { stub: true }) {
    ...
    async invoke(request) {
        ...
        return {
            allowed: true,
            tool_id: request.tool_id,
            result: { ...request.params, _result: this.resultPayload },
...
function resolveSandboxedPath(relativePath) {
...
const BUILTIN_EXECUTABLE_TOOLS = {
    stub_tool: {
    ...
    file_write_preview: {
        ...
        execute: async (request) => {
            ...
            if (!overwrite) {
                await writeFile(resolvedPath, content, { encoding: encoding, flag: "wx" });
            }
            else {
                await writeFile(resolvedPath, content, { encoding: encoding });
            }
            return {
...
export function toolIdRequiresNetworkAccess(toolId) {
...
export function toolIdRequiresFilesystemAccess(toolId) {
    return BUILTIN_EXECUTABLE_TOOLS[toolId]?.requires_filesystem === true;
}
export class ExecutableToolGateway {
    registry;
    constructor(registry = BUILTIN_EXECUTABLE_TOOLS) {
