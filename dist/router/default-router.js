...
export const defaultRouter = {
    plan(input) {
        ...
        const allowed = input.policy.allowed_pipelines ?? [];
        const useCodingAgent = allowed.includes("coding_agent") && (input.intent.complexity?.tool_likelihood ?? 0) >= 0.5;
        const useDeepResearch = allowed.includes("deep_research") &&
            (input.intent.routing_hints ?? []).includes("deep_research");
        const useDecision = allowed.includes("decision") && (input.intent.routing_hints ?? []).includes("decision");
        const useCompositeExample = allowed.includes("composite_example") &&
            (input.intent.routing_hints ?? []).includes("composite_example");
        const useToolAutomation = allowed.includes("tool_automation") &&
            (input.intent.routing_hints ?? []).includes("tool_automation");
        const useExtraction = allowed.includes("extraction") && (input.intent.routing_hints ?? []).includes("extraction");
        const useVerification = allowed.includes("verification") && (input.intent.routing_hints ?? []).includes("verification");
        const usePlanningOnly = allowed.includes("planning_only") && (input.intent.routing_hints ?? []).includes("planning_only");
        const useBatchAnalysis = allowed.includes("batch_analysis") && (input.intent.routing_hints ?? []).includes("batch_analysis");
