...
export class SecretScopeViolationError extends Error {
    ref;
    caller;
    constructor(message, ref, caller) {
        super(message);
        this.ref = ref;
        this.caller = caller;
 ...
 */
export function isSecretAllowedForCaller(ref, caller) {
    ...
    if (parts.length === 2) {
        ...
        if (orgOrRole === "org")
            return caller.orgId === value;
        if (orgOrRole === "env" && ref.env)
            return ref.env === value;
        if (orgOrRole === "role")
            return caller.scopes.includes(value);
 ...
 */
export async function resolveSecret(ref, caller) {
    if (!isSecretAllowedForCaller(ref, caller)) {
        incrementCounter(METRIC_SECRET_SCOPE_VIOLATIONS_TOTAL, 1);
        throw new SecretScopeViolationError(`Secret scope ${ref.scope} does not allow access for org=${caller.orgId}`, ref, caller);
    }
    return await Promise.resolve("[REDACTED]");
}
//# sourceMappingURL=secret-scope.js.map