...
import { FileVectorBackend, HashEmbeddingProvider, InMemoryVectorBackend, OpenAiEmbeddingProvider, VectorRetrievalAdapter, } from "./vector-retrieval-adapter.js";
...
function createEmbeddingProvider(config) {
    switch (config.provider) {
        case "openai": {
            ...
            if (!apiKey) {
                console.warn(`[memory] Embedding provider 'openai' selected but ${config.api_key_env} is not set. Falling back to hash provider.`);
 ...
 */
export function getMemoryGateway() {
    if (!gatewayInstance) {
        const retention = getMemoryRetentionConfig();
