...
const AuthClaimMappingSchema = z
    ...
    .default({});
const DEFAULT_AUTH_IDP_REGISTRY = [
    {
...
export function loadConfigFromEnv() {
    ...
    if (rolloutFromLegacy !== undefined &&
        rolloutFromCanonical !== undefined &&
        rolloutFromLegacy !== rolloutFromCanonical) {
        console.warn("[config] PLATFORM_MASTER_ROLLOUT_ENABLED conflicts with PLATFORM_PRODUCTION_ROLLOUT_ENABLED; canonical flag wins");
    }
    ...
    const memoryRetentionMaxChunks = process.env.MEMORY_RETENTION_MAX_CHUNKS_PER_SCOPE
        ...
        : undefined;
    const memoryRetention = memoryRetentionTtl !== undefined || memoryRetentionMaxChunks !== undefined
    ...
    const memoryMaxChunksPerIngest = process.env.MEMORY_MAX_CHUNKS_PER_INGEST
        ...
        : 128;
    const memoryIngestChunkCapPolicy = process.env.MEMORY_INGEST_CHUNK_CAP_POLICY === "reject" ? "reject" : "trim";
    ...
    const authQueryRequiredScopes = parseCsvEnv(process.env.AUTH_QUERY_REQUIRED_SCOPES) ?? ["query:invoke"];
    ...
    if (authIdpRegistry !== undefined)
        authConfig.idp_registry = authIdpRegistry;
    if (authAppRegistry !== undefined)
        authConfig.app_registry = authAppRegistry;
    const fromEnv = {
        ...raw,
        env,
