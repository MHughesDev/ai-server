...
function loadBaseline() {
    ...
    if (!Array.isArray(data))
        throw new Error("baseline.json must be an array of eval cases");
    return data;
}
async function main() {
    const cases = loadBaseline();
    const result = await runEvalHarness(cases);
    console.info(`Eval: ${result.passed}/${result.total} passed, ${result.failed} failed, ${result.duration_ms}ms`);
