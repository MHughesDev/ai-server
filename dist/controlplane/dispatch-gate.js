 ...
 */
export function assertCanDispatch(result) {
    if (!result.routeResult.allowed || !result.pipelinePlan) {
        const reason = !result.routeResult.allowed && "denyReason" in result.routeResult
            ...
            : "POLICY_BLOCKED";
        throw new DispatchBlockedError("Dispatch precondition failed: governance did not allow execution", reason);
    }
}
/** Thrown when dispatch is attempted without allow + pipeline plan */
export class DispatchBlockedError extends Error {
    denyReason;
    constructor(message, denyReason) {
        super(message);
 ...
 */
export function canDispatch(result) {
