...
export function parseRolloutPolicy(input) {
    ...
    const raw = input;
    const canary = raw.canary != null && typeof raw.canary === "object"
        ? parseCanaryThresholds(raw.canary)
        : DEFAULT_POLICY.canary;
    const rollback_allowed = typeof raw.rollback_allowed === "boolean" ? raw.rollback_allowed : true;
 ...
 */
export function parseCanaryThresholds(input) {
    const toNum = (v, def) => {
    ...
    const max_error_rate_promotion = toNum(input.max_error_rate_promotion, DEFAULT_POLICY.canary.max_error_rate_promotion);
    const abort_error_rate = toNum(input.abort_error_rate, DEFAULT_POLICY.canary.abort_error_rate);
    const max_p95_latency_ratio = toNum(input.max_p95_latency_ratio, DEFAULT_POLICY.canary.max_p95_latency_ratio);
    const observation_window_minutes = Math.floor(toNum(input.observation_window_minutes ?? input.observation_window_min, DEFAULT_POLICY.canary.observation_window_minutes));
