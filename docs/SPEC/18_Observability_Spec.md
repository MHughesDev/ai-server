...
- **Event/audit file sinks:** async `appendFile` + bounded queues + size-based rotation (`event-sink.ts`, `audit-logger.ts`). **Production (PR-012):** bootstrap requires **`AUDIT_LOG_PATH`** and **`OBSERVABILITY_EVENT_SINK_PATH`** (`assert-production-persistent-sinks.ts`); parent dirs writable (`sink-paths.ts`). Rotation via **`AUDIT_LOG_MAX_FILE_BYTES`** / **`AUDIT_LOG_MAX_ROTATED_FILES`** and **`OBSERVABILITY_EVENT_SINK_*`** env vars (`file-sink-options.ts`, `wirePersistentSinksFromConfig`). Optional **`AUDIT_LOG_FSYNC`** / **`OBSERVABILITY_EVENT_SINK_FSYNC`** → per-append `fsync`. **Shutdown:** drain audit queue + event sink `close()` on `SIGINT`/`SIGTERM` (`server/index.ts`).
