...
- **L2-99 gate flag (historical plan name):** `governance_harness_readiness_gate_active` is **not** implemented in `src/config/schema.ts` (2026-03-24 reconciliation). The gate is **process + evidence**, not a runtime toggle. Autonomous harness behavior uses **`harness_autonomous_execution_enabled`** — see **`docs/SPEC/20_Config_and_FeatureFlags.md`**.
...
- `docs/PLANS/implementation/L2-08_Rollout-and-Operational-Readiness-Implementation.md` complete.
...
## 10) Rollout Strategy
### 10.1 Feature Flags
- **L2-99 readiness gate:** **Process-only** — no `FeatureFlagsSchema` key. Older drafts referenced `governance_harness_readiness_gate_active`; that name is **not** loaded from config in this codebase (2026-03-24).
- **Autonomous harness (when product enables it):** **`harness_autonomous_execution_enabled`** / **`HARNESS_AUTONOMOUS_EXECUTION_ENABLED`** — see SPEC 20 and coding-agent pipeline.
- Rollout criteria: Gate = governance cadence; harness flag = product/env configuration.
...
## 14) Sprint handoff archive (L2-99 → pilot / remediation)

*Merged from the former `L2-99_Handoff.md`.*
