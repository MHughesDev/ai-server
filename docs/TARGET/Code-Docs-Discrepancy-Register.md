...
| D-009 | MVP disabled response | `routes.ts`: `runtime_mvp_query_chat_enabled` false → **404** `{ error: "MVP query endpoint not enabled" }` (not `ResponseEnvelope`). | **Closed 2026-03-24** — SPEC 02 + `openapi.yaml` `404` + `MvpQueryDisabledResponse`. | P2 | **Fixed** (docs + OpenAPI). |
